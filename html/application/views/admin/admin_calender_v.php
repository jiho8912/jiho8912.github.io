...
$(document).ready(function(){
	...
	$("[id*='reg_schedule_']").each(function( index ) {
		$(this).click(function() {
			var year = "<?=$nowYear;?>";
			var month = "<?=$nowMonth;?>";
			var day = $(this).val();

			location.replace("/admin/calender/reg_schedule_v?year="+year+"&month="+month+"&day="+day);
		});
	});

	$("[id*='update_schedule_']").each(function( index ) {
		$(this).click(function() {
			var year = "<?=$nowYear;?>";
			var month = "<?=$nowMonth;?>";
			var day = $("#reserv_select_day").text(); //일자 표시
			var no = $(this).val();

			location.replace("/admin/calender/reg_schedule_v?year="+year+"&month="+month+"&day="+day+"&no="+no);
		});
	});

	$("[id*='delete_schedule_']").each(function( index ) {
		$(this).click(function() {
			var no = $(this).val();
			var result = confirm('삭제하시겠습니까?');

			if(result) {
				location.replace("/admin/calender/delete_schedule_data?no="+no);
			} else {
				//no
			}
		});
				...
				<? if($list['day']){?>
					<div class="calendar_area">
						...
						<div style = "margin-top:10px;">
							<button type="button" id="reg_schedule_<?=$list['day']?>" class="btn btn-sm btn-primary" value = "<?=$list['day']?>">일정등록</button>
						</div>
					</div>
				<? }?>
				<!-- 달력 구조 --></td>
